...
def test_two_state_absorption_against_fortran_code(two_state_params_fortran):
    params = two_state_params_fortran
    absorption = ts.absorption(params)

    absorption = absorption.match_greatest_peak_of(FORTRAN_CODE_ABSORPTION)
    assert jnp.allclose(absorption.intensities, FORTRAN_CODE_ABSORPTION)
